...
User <- struct(
  .name = "User",
  .description = "A description of the structure User",
  id   = int[1][. > 0]                            %doc% "The ID of the user.",
  name = chr[1]                                   %doc% "Full name of the user.",
  role = one_of("admin", "user") %default% "user" %doc% "Description of role.",
...
When nothing built in fits, `type_from()` takes a plain predicate and an
optional coercion:
...
upload — use `try_parse()`, which returns the same records instead of
throwing:
...
`to_json()` is type-aware: a field declared `[1]` becomes a JSON scalar,
any other vector becomes an array, even when it has one element.
...
`from_json()` parses and validates in one step. JSON arrays come back as
R vectors and arrays of objects as data frames, because the types accept
...
actually reads. For concise documentation, `desc()` (or its infix
spelling `%doc%`) documents a single field’s type, and
...
prose in `describe()`. Its `.` argument describes the struct; every
other argument is named for a field. Assign the result back to the
struct name, as with any base R pipe that transforms an object.
...
## Possible extensions
  ...
  - An actual ORM system for R
  - Extension/examples to make it compatible with ellmer for structured
    output
